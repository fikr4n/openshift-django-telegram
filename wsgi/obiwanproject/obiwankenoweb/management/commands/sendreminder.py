...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            last_t = datetime.strptime(Misc.objects.get(key='last_reminder_time').value, _DB_DATETIME_FORMAT)
        ...
        Misc.objects.update_or_create(key='last_reminder_time', defaults={
            'value': now_t.strftime(_DB_DATETIME_FORMAT)})
